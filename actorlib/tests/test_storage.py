...
def make_wal_items(msg_id, status=None):
    ...
    items = [
        ...
        op('ack', msg_id, ack_message_id=msg_s1, status='OK'),
        op('ack', msg_id, ack_message_id=msg_s2, status='ERROR'),
        op('retry', msg_id, ack_message_id=msg_s2),
        op('ack', msg_id, ack_message_id=msg_s2, status='ERROR'),
    ]
...
def test_actor_state():
    ...
    wal_items = [
        ...
        op('ack', 'MSG_ID', ack_message_id='S1', status='OK'),
        op('ack', 'MSG_ID', ack_message_id='S2', status='ERROR'),
        op('retry', 'MSG_ID', ack_message_id='S2'),
    ]
    state.load(wal_items)
    assert state.query_send_message_ids() == {'S1', 'S2'}
    ...
    state.apply(op('ack', 'MSG_ID', ack_message_id='S2', status='OK'))
    assert msg['status'] == 'OK'
    assert state.query_send_message_ids() == set()
